...
def handle_tentukan_pelanggan_terbaik(entities):
    ...
    for i, status_req in enumerate(status_list):
        ...
        # Jalankan fungsi Weighted Product
...
# WEIGHTED PRODUCT
# =========================
def weigted_product_proses(tanggal_awal, tanggal_akhir, limit, status):
    conn = connection_db()
    cursor = conn.cursor(dictionary=True)
    ...
    try:
        ...
        return grouping_logic(hasil_wp, limit, status, tanggal_awal, tanggal_akhir)
...
# GROUPING LOGIC (FIX)
# =========================
def grouping_logic(hasil_wp, limit, status, tanggal_awal, tanggal_akhir):
    ...
    threshold_sangat = max_nilai * 0.9

    sangat_terbaik = [x for x in hasil_wp if x["nilai_wp"] >= threshold_sangat]
    terbaik = [x for x in hasil_wp if mean <= x["nilai_wp"] < threshold_sangat]
    biasa = [x for x in hasil_wp if x["nilai_wp"] < mean]
    ...
    target_key = status_map.get(status.lower(), "terbaik")

    # 🔥 FIX INTENSITY
    if "sangat" in status.lower():
        target_key = "sangat_terbaik"

    # 🔥 FIX GROUPING
    if target_key == "sangat_terbaik":
        data_final = sangat_terbaik[:limit]

    elif target_key == "terbaik":
        data_final = terbaik[:limit]

    else:
        data_final = sorted(biasa, key=lambda x: x["nilai_wp"])[:limit]
    ...
    # 🔥 batasi history biar tidak penuh
